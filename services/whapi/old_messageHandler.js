...
async function handleImage(message, whapi) {
 ...
 console.log("Image Upload 2");
  ...
  // 🖼️ IMAGE (blur check)
  // ============================================================
  //console.log("Check Mime Type: ",mime);
  ...
  // ============================================================
  if (mime === "text/csv" || mime.includes("excel") || mime.includes("spreadsheet"))
  {
    await whapi.sendText(
      chat_id,
...
async function handleInteractive(message, whapi) {
  ...
  if (btnId === "user_driver") {
    ...
    // Default buttons
    // const driverButtons = [];
    // const arrDriverDocTypeByName=[];
    // let arrDriverDocType =[];

    // const strDocTypeQuery = `select id,name FROM m_required_document where is_active='Y' AND doc_type IN('B','T')`;
    // arrDriverDocType = await db.executeQuery(message.databaseName,strDocTypeQuery, {  }, true);
    // console.log("arrDocType", arrDriverDocType);
    
    // // Push DB results dynamically
    // if (arrDriverDocType && arrDriverDocType.length > 0) {
    //   arrDriverDocType.forEach(doc => {
    //     driverButtons.push({
    ...
    //   });
    // }
    // return whapi.sendButtons(chat_id, msg.selectDoc, driverButtons);
...
async function handleReply(message, whapi) {
  ...
  if (["ButtonsV3:doc_pod", "ButtonsV3:doc_receipt","ButtonsV3:doc_other"].includes(btnId)) {
    ...
    if(btnId=="ButtonsV3:doc_pod" || btnId=="ButtonsV3:doc_receipt" || btnId=="ButtonsV3:doc_other"){
      ...
      if(btnId=="ButtonsV3:doc_receipt"){
         docName = 'Receipt';
      }
      if(btnId=="ButtonsV3:doc_other"){
         docName = 'Other';
      ...
      const strQueryDoc =`SELECT * FROM m_required_document WHERE name ='${docName}' `;
      const resultDoc = await db.executeQuery(message.databaseName, strQueryDoc, {  }, true);
      const documentType = resultDoc?.length > 0 ? resultDoc[0].id : null;
